...
namespace api.Infrastructure.DataAnnotationsModelValidation
{
    ...
    internal class DataAnnotationsModelValidator : IModelValidator
    {
        ...
        public IEnumerable<ModelValidationResult> Validate(ModelValidationContext validationContext)
        {
            ...
            var test1 = Attribute is MyCustomValidationAttribute;
            var result = Attribute.GetValidationResult(validationContext.Model, context);
            var test2 = result is MyCustomValidationResult;
            var test3 = (result as MyCustomValidationResult).MyProperty;

            if (result != ValidationResult.Success)
            {
                string errorMessage = "MY_PREFIX: ";
                ...
                var validationResults = new List<CustomModelValidationResult>();
                if (result.MemberNames != null)
                {
                    foreach (var resultMemberName in result.MemberNames)
                    {
                        ...
                        var newMemberName = string.Equals(resultMemberName, memberName, StringComparison.Ordinal) ?
                            null :
                            resultMemberName;
                        var validationResult = new CustomModelValidationResult(newMemberName, errorMessage);
                ...
                if (validationResults.Count == 0)
                {
                    ...
                    validationResults.Add(new CustomModelValidationResult(memberName: null, message: errorMessage));
                }
